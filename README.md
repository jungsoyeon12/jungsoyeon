#### 20201119 목
html영역지정방식: tag, class, id
id방식은 현재 html문서에서 1개만 존재해야 함
class는 여러개 존재해도 가능함
...
태그사용 기본형식: <태그명> 내용</태그>
index.html 의미: 홈페이지 진입점, 자동 실행됨
자바프로그램 실행: main () 매서드 진입점, 자동 실행되는 진입점입니다

 #### 20201117 화
카카오오븐에서 화면설계하기.
화면장표(필수): 화면이름, 화면ID, 화면경로(링크경로), 관련계정, 메뉴바출력여부, 비고(픽사베이:무료이미지)
 #### 20201113 금
 스프링 MVC구조 중 controller 단 homeconntroller..java 자바 클래스를 분석해 보겠습니다.
 ===================================================== 
 homecontroller 분석해 봅니다.
 클래스의 역할 표시: 아래 3가지 콤포넌트
 @Controller 지시자 아래클래스는 컨트롤러 역할이다라고 스프링에 명시
 @Service  프로그램로직구현 역할이다라고 스프링에 명시
 @Repository 지시자는 데이터베이스 Model을 조작하는 역할이다라고 스프링에 명시
 참고로 html 주석은 <!-- 주석내용 -->
 자바주석: 2가지 아래
 1줄 주석: //로 시작
 여러줄 주석: /* ... */
- @import: 애노테이션 임포트지시어 외부 라이브러리
 -package: 패키지, 자바소스코드를 관리하기 위해서 만든 가상영역
 -home.jsp : MVC프로젝트중 View단 home.jsp 화면분석해보았습니다.
 -java 자바: 오라클에서 제작한 라이센스가 있는 프로그래밍 언어
 java폴더 안에 .org폴더안 .edu폴더안 .controller 폴더 안에 자바소스 파일이 생성됨
 ======================================================
 자바로 만든 홈페이지가 jsp 임. MVC중 view단 분석
 jsp(jva server page): 스프링 MVC프로젝트 신규 생성(새문서생성)시 아래 기본출력
 꺽쇠<% .... %> 조건은 jsp파일에서 자바프로그램을 사용할때 <% 자바프로그램 %>
   <%@ taglib 애노테이션 태그라이브러리 지시자(어) 역할: 페이지 인코딩(한글처리)
   taglib = url ("외부자바프로그램용 태그를 가져다가 사용할 주소")
   prefix(접두사-축약어) = "fn"
   <%@ page session : 로그인 유지할때 사용.
   세션: 원래 웹페이지는 페이지 이동시 로그인정보 사라짐. 이런 정보를 방지하기 위해
   기본은 session = "true" 세션 변수를 사용하겠다는 의미이고, 기본값)
   -팁 이클립스에서 저장 ctrl+S 되돌리기 Ctrl+z
 1. 헤로쿠에서 hsql 메모리  데이터베이스 사용 (서버 껐다키면, 원상복귀)
 2. 파스타에서는 mysql 데이터베이스 사용 (서버를 껐다가 켜도 유지됨)
 -encoding(코드화, UTF-8)<->decoding(복호화)
 작업순서
 1. 이클립스에서 학생명의 프로젝트 소스수정/기능추가
 2. 이클립스에서 수정한 결과, Run on sever로 톰캣에서 확인 ok (http://localhost:8080.
 3. 깃에 푸시
 4. 헤로쿠에서 Deploy Branch(깃브렌치) 클릭 -> 깃에 있는 소스를 헤로쿠 클라우드로 배포하는것
 5. 헤로쿠에서 open app: https://jungsoyeon.herokuapp.com/ (나중에 이력서 포트폴리오로 씀)
 - 실습4 헤로쿠 클라우드에 프로젝트 배포, 도메인rul로 크롬에서 확인예정
 - 아마존 웹서비스 aws도 마찬가지, 클라우드파스타와 마찬가지로 헤로쿠도 클라우드 설정파일을 작성해야지만, 작동이 됩니다.
 깃에 올릴때, 제외시킬 폴더를 지정. target폴더는 깃에서 제외시키는 방법
 .gitignore 파일을 만들어서, target 이름을 적으시면 됩니다.
 - root 루트: 최상위 위치를 가리킵니다.
 - POM.xml 파일에서 버전을 변경하게 됩니다
  - program에 사용되는 object module(외부프로그램) 관리하는 파일을 pom.xml라고 함 
  - 깃서버와 이클립스 연동
  - 깃 저장 순서: 커밋(.git폴더에 저장)+ 푸시(github.com 에 저장)
 - 형상관리=버전관리=소스관리=팀작업
 - 이클립스 사용법:
  퍼스펙티브(perspective): 개발자관점.(java,eGov개발자관점)
  관점 리셋: window-perspective-reset~
 - 파일저장:화면상단 메뉴아이콘중 디스켓모양 / ctrl+s
#### 응용SW기초기술활용과목(4개의 단원)
- 4단원: 개발환경 구축하기 시작
